...
bool ReadingWriting::writeOutput(string outputFileName, vector<Process>& processes)
{
	...
	for (int i = 0; i < n ; i++)
	{
		outputFile << setprecision(2) << processes[i].processNumber << " " << processes[i].arrivalTime << "  " << processes[i].burstTime << "  " << processes[i].priority << endl;
	}
...
bool ReadingWriting::writingFinishingOutputFile()
{
	...
	cout << "See Files results.txt and Statistics.txt " << endl;
